<script lang="ts">
	...
	$effect(() => {
		if (results.weeklyCharges > 0) {
			...
			// Check if a safety charge was added due to range being close to distance
			const threshold = 0.1;
			const ratio = formData.weeklyDistanceKm / results.effectiveRangeKm;
			safetyChargeAdded = ratio > 1 - threshold && ratio < 1;
		} else {
			annualCost = 0;
			co2Savings = 0;
